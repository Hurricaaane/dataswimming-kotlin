...
internal class TessellationTest {
    ...
    fun tessellating() {
        ...
        assertThat(
                Tessellation.from(Treemapping(listOf(1, 1), { it.toLong() }), 0).toSet(),
                `is`(setOf(
        ...
        assertThat(
                Tessellation.from(Treemapping(listOf(1, 1), { it.toLong() }), 1).toSet(),
                `is`(setOf(
        ...
        assertThat(
                Tessellation.from(Treemapping(listOf(2, 1, 1), { it.toLong() }), 0).toSet(),
                `is`(setOf(
